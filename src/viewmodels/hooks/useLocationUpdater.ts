...
export function useLocationUpdater({
  ...
  const { latitude, longitude, isLoading, requestLocation } =
    useRequestLocation();
  useEffect(() => {
    if (isLoading) return;
    updateLocation(userId, new LatLng(latitude ?? 0, longitude ?? 0));
  ...
  }, [userRole, updateLocation, requestLocation, isLoading, userId]);
